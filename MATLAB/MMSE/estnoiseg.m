 ...
 %
 % Usage:    ninc=round(0.fs);   % frame increment [fs=sample frequency]
 ...
 %
 %                   (a) dp=estnoiseg(yp(1:3tinc);
 %
 %                   (b) [dp(1:1),z]=estnoiseg(yp(1:1),tinc);
 %                       [dp(1),z]=estnoiseg(yp(1),z);
 %                       [dp(2),z]=estnoiseg(yp(2),z);
 ...
 %       Copyright (C) Mike Brookes 2 %      Version: $Id: estnoiseg.m 3387 208-23 12:32:47Z dmb $
 %
 ...
 if isempty(yf) && isstruct(tz)             % no real data
     ...
     if isstruct(tz)         % take parameters from a previous call
         ...
         qq.pnsaf=0.     % noise probability safety value [0.(24)
     ...
     pnsaf=qq.pnsaf;     % noise probability safety value [0.(24)
     ...
     if ~nargout
         ...
         xlabel(sprintf('Time (s)   [%d ms frame incr]',round(tinc*1)));
         axisenlarge([-1 -1.05]);
