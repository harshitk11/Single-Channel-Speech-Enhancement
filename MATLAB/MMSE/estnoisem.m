 ...
 %
 %                   (a) dp=estnoisem(yp(1:3tinc);
 %
 %                   (b) [dp(1:1z]=estnoisem(yp(1:1tinc);
 %                       [dp(101:2z]=estnoisem(yp(101:2z);
 %                       [dp(201:3z]=estnoisem(yp(201:3z);
 ...
 %        Signal Processing, 2 86, 1215-1229
 %    [3] Dirk Mauler and Rainer Martin
 %        Noise power spectral density estimation on highly correlated data
 %        Proc IWAENC, 2 
 %       Copyright (C) Mike Brookes 2 %      Version: $Id: estnoisem.m 1718 2012-03-31 16:40:41Z dmb $
 %
 ...
 if isempty(yf) && isstruct(tz)             % no real data
     ...
     if isstruct(tz)       % take parameters from a previous call
         ...
         qq.taminh=0.;    % min smoothing time constant (upper limit) in (3) = -tinc/log(0.3)
